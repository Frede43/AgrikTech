...
export function Sidebar({ mobile = false }: SidebarProps) {
  ...
  const navItems = [
    ...
    { href: "/fermier/credits", label: lang === "fr" ? "Crédit Agricole" : "Ideni ry'uburimyi", icon: Wallet },
    { href: "/fermier/cooperative", label: lang === "fr" ? "Ma Coopérative" : "Ishirahamwe ryanje", icon: Leaf },
    { href: "/fermier/messages", label: lang === "fr" ? "Messagerie" : "Ubutumwa", icon: MessageCircle },
  ...
  return (
    <aside className={cn(
      ...
      <div className="px-3 py-4 border-t border-sidebar-border space-y-0.5">
        ...
        <Link href="/fermier/guide" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-foreground transition-colors">
          <BookOpen className="w-4 h-4" />
          {lang === "fr" ? "Guide d'utilisation" : "Ubuyobozi bwo gukoresha"}
        </Link>
