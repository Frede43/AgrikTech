...
def create_order(order: schemas.OrderCreate, request: Request, db: Session = Depends(get_db)):
    ...
    if not product or float(str(product.quantity_kg)) < order.quantity:
        raise HTTPException(status_code=400, detail="Stock insuffisant.")
        
    total_price = config.Decimal(str(product.price_per_kg)) * config.Decimal(str(order.quantity))
    vat_rate = config.Decimal(str(product.vat_rate)) if product.vat_rate is not None else config.Decimal("0.18")
    is_taxable_val = bool(product.is_taxable)
    subtotal = total_price / (config.Decimal("1") + vat_rate) if is_taxable_val else total_price
    ...
    product.quantity_kg = float(str(product.quantity_kg)) - order.quantity # type: ignore
    db.commit()
    db.refresh(db_order)
    return db_order
...
def get_order_detail(order_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    is_buyer = bool(int(str(order.buyer_id)) == user.id)
    is_farmer = bool(int(str(order.farmer_id)) == user.id)
    is_driver = bool(int(str(order.driver_id)) == user.id) if order.driver_id is not None else True # Le livreur peut voir les commandes sans livreur (pool)
    ...
    return {
        "id": int(str(order.id)),
        "orderId": utils.format_order_reference(int(str(order.id))),
        "status": str(order.status).lower(), # Le frontend attend du minuscule pour statusLabels
        "farmer": {
        ...
        "items": [
            {
                "name": str(product.name) if product and product.name else "Produit",
                "qty": float(str(order.quantity)) if order.quantity is not None else 0.0,
                "unit": str(product.unit) if product and product.unit else "kg"
            }
        ],
        "totalWeight": f"{float(str(order.quantity)) if order.quantity is not None else 0.0} {str(product.unit) if product and product.unit else 'kg'}",
        "distance": utils.format_distance_label(dist_km),
        "estimatedDuration": utils.format_duration_label(dist_km),
        "instructions": "Fragile - Manipuler avec soin" if "tomate" in (str(product.name).lower() if product and product.name else "") else "Livraison standard",
        "pickup_qr": str(order.pickup_qr_token) if order.pickup_qr_token else "—",
        "delivery_otp": str(order.delivery_otp) if order.delivery_otp else "—"
    }
...
def get_buyer_orders(buyer_id: int, db: Session = Depends(get_db)):
    ...
    for o in orders:
        ...
        results.append({
            ...
            "driver": driver_info,
            "items": items,
            "total": float(str(o.total_price)) if o.total_price is not None else 0.0,
            "totalWeight": f"{float(str(o.quantity)) if o.quantity is not None else 0.0} kg",
