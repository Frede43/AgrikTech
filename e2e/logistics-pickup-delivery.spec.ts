...
test("driver can pick up and deliver an order", async ({ page, request }) => {
  ...
  const initialDetailResponse = await request.get(`${API_URL}/testing/orders/${order.id}`);
  ...
  expect(initialDetail.status).toBe("paid_escrow");

  await loginViaOtp(page, "logistique", driverPhone.replace("+257", ""));

  await expect(page.getByText("Commandes disponibles")).toBeVisible();
  ...
  await expect(page.getByText("Mes livraisons")).toBeVisible();
  await expect(page.getByText(product.name)).toBeVisible();

  const collectedDetailResponse = await request.get(`${API_URL}/testing/orders/${order.id}`);
  expect(collectedDetailResponse.ok()).toBeTruthy();
  const collectedDetail = (await collectedDetailResponse.json()) as {
    status: string;
    delivery_otp: string;
  };
  expect(collectedDetail.status).toBe("picked_up");
  ...
  const deliveredDetailResponse = await request.get(`${API_URL}/testing/orders/${order.id}`);
  expect(deliveredDetailResponse.ok()).toBeTruthy();
  const deliveredDetail = (await deliveredDetailResponse.json()) as { status: string };
  expect(deliveredDetail.status).toBe("completed");

  const logisticsOrdersResponse = await request.get(`${API_URL}/orders/logistics?mode=pool`);
  ...
  const farmerTransactionsResponse = await request.get(`${API_URL}/testing/users/${farmer.id}/transactions`);
